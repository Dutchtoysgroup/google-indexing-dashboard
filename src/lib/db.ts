...
export const getAllShopsSummary = unstable_cache(
  async (): Promise<ShopSummary[]> => {
    const sql = getDb();
  ...
  ["all-shops-summary"],
  { tags: [DASHBOARD_TAG], revalidate: REVALIDATE_SECONDS }
);

export const getShopSummary = unstable_cache(
  async (shopId: string): Promise<ShopSummary | null> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
        shop_id,
...
export const getShopSnapshots = unstable_cache(
  async (shopId: string, days: number = 30): Promise<DailySnapshot[]> => {
    const sql = getDb();
    const rows = await sql`
...
export const getAllSnapshots = unstable_cache(
  async (days: number = 30): Promise<DailySnapshot[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
        date::text,
...
export const getApiLog = unstable_cache(
  async (days: number = 30): Promise<ApiLogEntry[]> => {
    const sql = getDb();
    const rows = await sql`
...
export const getApiLogByShop = unstable_cache(
  async (shopId: string, days: number = 30): Promise<ApiLogEntry[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT id, date::text, shop_id, api_type, url_count, created_at::text
      FROM api_log
...
export const getApiDailySummary = unstable_cache(
  async (days: number = 30): Promise<ApiDailySummary[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
        date::text,
...
export const getApiDailySummaryByShop = unstable_cache(
  async (shopId: string, days: number = 30): Promise<ApiDailySummary[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
        date::text,
  ...
  ["api-daily-summary-by-shop"],
  { tags: [DASHBOARD_TAG], revalidate: REVALIDATE_SECONDS }
);

export const getTodayApiUsage = unstable_cache(
  async (): Promise<TodayUsage> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
...
export const getLastRunEndedAt = unstable_cache(
...
export const getRecentlyPushedUrls = unstable_cache(
  async (shopId: string, limit: number = 10): Promise<UrlRow[]> => {
    const sql = getDb();
    const rows = await sql`
  ...
  ["recently-pushed-urls"],
  { tags: [DASHBOARD_TAG], revalidate: REVALIDATE_SECONDS }
);

export const getShopExtraStats = unstable_cache(
  async (shopId: string): Promise<ShopExtraStats> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
...
export const getCoverageStateBreakdown = unstable_cache(
  async (): Promise<CoverageStateEntry[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
...
export const getApiUsageByShop = unstable_cache(
  async (days: number = 30): Promise<ApiUsageByShop[]> => {
    const sql = getDb();
    const rows = await sql`
      SELECT
        shop_id,
...
export const getShopUrls = unstable_cache(
  async (
    shopId: string,
    filters: {
      type?: string;
